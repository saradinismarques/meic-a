...
def check_password(filename: str, password: str):
    ...
    with open((f"{filename}"), mode='rb') as (encoded_source):
        with b64(encoded_source) as (source):
            ...
            for count, passw in enumerate(password_list):
                passw = passw[:-1]

                get_password_hash(passw)
                if hash == get_password_hash(passw):
                    print(f'{count} Password {passw} is correct!')
                    exit(1)
                else:
                    print(f'{count} Password {passw} is incorrect! Aborting...')
            exit(1)
